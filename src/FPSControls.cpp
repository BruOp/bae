...
namespace bae
{
FPSControls::FPSControls(Camera& camera, const float sensitivity, const float movementSpeed)
    : pCamera{&camera}, sensitivity{sensitivity}, movementSpeed{movementSpeed} {};

EventHandleResult FPSControls::handleEvents(const EventQueue& eventQueue)
{
...
void FPSControls::handleMouseMovement(const SDL_Event& event)
{
    ...
    pitch = glm::clamp<float>(pitch - event.motion.yrel * sensitivity, s_lowerPitchLimit, s_upperPitchLimit);
}

void FPSControls::update(const float dt)
{
    glm::vec3 front{glm::cos(yaw) * glm::cos(pitch), glm::sin(pitch), glm::cos(pitch) * glm::sin(yaw)};
    ...
    if (glm::length(currentDirection) != 0) {
        glm::vec3 movementVector = glm::normalize(
            ...
            currentDirection.y * pCamera->WorldUp);
