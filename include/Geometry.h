...
struct Geometry {
    bgfx::VertexBufferHandle vertexBuffers[MAX_VERT_BUFFERS_PER_GEOMETRY] = {BGFX_INVALID_HANDLE,
                                                                             BGFX_INVALID_HANDLE,
                                                                             BGFX_INVALID_HANDLE,
                                                                             BGFX_INVALID_HANDLE};
...
class GeometryRegistry
{
   public:
    ...
    inline Geometry get(const std::string& name) const { return geometry_map.at(name); }

    inline Geometry& set(const std::string& name, const Geometry& geometry) { return geometry_map[name] = geometry; }

    template <typename Vertex>
    Geometry create(const std::string& name, const std::vector<Vertex>& vertices, const std::vector<uint16_t>& indices)
        noexcept
    {
        Geometry geometry{};
        const bgfx::Memory* vertMemory = bgfx::copy(vertices.data(), sizeof(vertices[0]) * vertices.size());
        geometry.numVertBufferStreams = 1;
        geometry.vertexBuffers[0] = bgfx::createVertexBuffer(vertMemory, Vertex::ms_declaration);
