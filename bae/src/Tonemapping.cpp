...
namespace bae
{
    const std::string ToneMapping::histogramProgramName = "cs_luminance_histogram";
    const std::string ToneMapping::averagingProgramName = "cs_luminance_average";
    const std::string ToneMapping::toneMappingProgramName = "tonemapping";


    void ToneMapping::init()
    {
        histogramProgram = ShaderUtils::loadProgram(histogramProgramName.c_str(), nullptr);
        averagingProgram = ShaderUtils::loadProgram(averagingProgramName.c_str(), nullptr);
        tonemappingProgram = ShaderUtils::loadProgram(toneMappingProgramName);
        ...
        s_hdrTexture = bgfx::createUniform("s_texColor", bgfx::UniformType::Int1);
        s_texAvgLuminance = bgfx::createUniform("s_texAvgLuminance", bgfx::UniformType::Int1);
    ...
    void ToneMapping::render(bgfx::TextureHandle hdrFbTexture, const ToneMapParams & toneMapParams, const float deltaTime)
    {
        ...
        float tonemap[4] = { 0.18, bx::square(3.0), 0.0, 0.0 };
