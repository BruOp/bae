...
class EventQueue
{
    ...
    enum PumpResult { RESULT_SUCCESS, RESULT_EMPTY, RESULT_FULL };

   public:
    EventQueue() : EventQueue(EventQueue::defaultSize) {}

    EventQueue(size_t bufferSize) : m_bufferSize{bufferSize}, m_queue(bufferSize) {}
    ...
    const_iterator begin() const { return m_queue.cbegin(); };

    const_iterator end() const { return m_queue.cbegin() + m_end; };

    size_t size() const { return m_end; }

    bool isFull() const { return m_end == m_bufferSize - 1; }
